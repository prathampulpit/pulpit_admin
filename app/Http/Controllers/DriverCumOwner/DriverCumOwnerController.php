...
namespace App\Http\Controllers\DriverCumOwner;
...
use App\Repositories\DriverCumOwnerRepository;
...
class DriverCumOwnerController extends Controller
{
    ...
    public function __construct(
        ...
        DriverCumOwnerRepository $driverCumOwnerRepository,
        TripRepository $tripRepository
    ) {
        $this->userRepository = $userRepository;
        $this->vehiclesRepository = $vehiclesRepository;
        $this->driverCumOwnerRepository = $driverCumOwnerRepository;
    ...
    public  function index($panel, Request $request, $filter = null)
    {
        ...
        if (in_array("4", $role_id_arr) || $user_role == 'administrator') {
            ...
            $countcompany['record_type'] = 'paid_user';
            //echo "<pre>"; print_r($countcompany); exit;
            $paid_users = $this->driverCumOwnerRepository->getByParamsForCount($countcompany);

            $countcompany = array();
            $countcompany['count'] = true;
            $countcompany['today_joined'] = $today;
            $countcompany['type_id'] = '5';
            $today_joined = $this->driverCumOwnerRepository->getByParamsForCount($countcompany);
            ...
            $yesterday_joined = $this->driverCumOwnerRepository->getByParamsForCount($countcompany);
            ...
            $week_joined = $this->driverCumOwnerRepository->getByParamsForCount($countcompany);
            ...
            $this_month_joined = $this->driverCumOwnerRepository->getByParamsForCount($countcompany);
            ...
            $last_month_joined = $this->driverCumOwnerRepository->getByParamsForCount($countcompany);

            //$all_joined = User::count();
            $countcompany = array();
            $countcompany['count'] = true;
            $countcompany['type_id'] = '5';
            $all_joined = $this->driverCumOwnerRepository->getByParamsForCount($countcompany);
            ...
            $countcompany['vehicle_type_id'] = $request->vehicle_type_id;
            $countcompany['plan_id'] = $request->plan_id;
            $active_users = $this->driverCumOwnerRepository->getByParamsForCount($countcompany);
            ...
            $countcompany['vehicle_type_id'] = $request->vehicle_type_id;
            $countcompany['plan_id'] = $request->plan_id;
            $inactive_users = $this->driverCumOwnerRepository->getByParamsForCount($countcompany);
            ...
            $countcompany['vehicle_type_id'] = $request->vehicle_type_id;
            $countcompany['plan_id'] = $request->plan_id;
            $all = $this->driverCumOwnerRepository->getByParamsForCount($countcompany);

            //$paid_users = UserPurchasedPlans::groupBy('user_id')->count();

            $unpaid_users = $all - $paid_users;
            ...
            $countcompany['vehicle_type_id'] = $request->vehicle_type_id;
            $countcompany['plan_id'] = $request->plan_id;
            $expired_users = $this->driverCumOwnerRepository->getByParamsForCount($countcompany);
            ...
            $cities = Cities::all();
            return view('admin.modules.driver_cum_owner.index', compact('file_path', 'user_type', 'today_joined', 'yesterday_joined', 'week_joined', 'this_month_joined', 'last_month_joined', 'all_joined', 'active_users', 'inactive_users', 'paid_users', 'unpaid_users', 'expired_users', 'uninstall_users', 'day_name', 'graph_trans_data', 'states', 'plans', 'vehicle_types', 'state_id', 'vehicle_type_id', 'plan_id', 'cities', 'filter'));
        } else {
            abort(403);
        }
    }
    ...
    {
        $user = Auth::user();
        //$in = $this->driverCumOwnerRepository->hieararchy($user, false);
        if (request('per_page') == 'all') {
            $countcompany = [];
            $countcompany['count'] = true;
            $total = $this->driverCumOwnerRepository->getByParams($countcompany);
        } else {
            $total = request('per_page', config('custom.db.per_page', 100));
        }
        $params['per_page'] = $total;
        $params['filter'] = $filter;
        $users = $this->driverCumOwnerRepository->getPanelPartnerUsers($request, $params);
        return $users;
    }
    ...
    public function createEdit($panel, $id = null)
    {
        ...
        if (in_array("4", $role_id_arr) || $user_role == 'administrator') {
            ...
            if ($id) {
                ...
                $user = $this->driverCumOwnerRepository->getByParams($params);
    ...
    public function store(Request $request)
    {
        ...
        if (!empty($request->get('id'))) {
            ...
            return redirect(route('admin.driver.otp', ['panel' => Session::get('panel'), 'id' => $user_id]))->withMessage($message);
        }
    ...
    public function show($panel, $id)
    {
        ...
        $params['response_type'] = "single";
        //$user = $this->driverCumOwnerRepository->getByParams($params);
        $user = $this->driverCumOwnerRepository->getUserDetails($params);
        /* echo "<pre>";
        print_r($user);
        exit; */
        ...
        $user_type_id = $user->user_type_id;
        $user_work_profiles_for_agent = UserWorkProfile::where('user_id', $id)->where('status', '1')->whereIn('user_type_id', array(2, 3))->first();
        ...
        $user_work_profiles_for_drivers = UserWorkProfile::where('user_id', $id)->where('status', '1')->whereIn('user_type_id', array(4, 5))->first();
        ...
        if (!empty($user_work_profiles_for_drivers)) {
            ...
            $driver_details = Drivers::where('id', $profile_id)->where('status', 1)->first();
        }

        $vehicle_details = Vehicles::where('user_id', $id)->where('status', 1)->first();
        /* echo "<pre>";
        ...
        $brands = array();
        if (isset($vehicle_details->brand_id)) {
            $brand_id = $vehicle_details->brand_id;
            $brands = VehicleBrands::where('id', '=', $brand_id)->first();
        }

        $models = array();
        if (isset($vehicle_details->model_id)) {
            $model_id = $vehicle_details->model_id;
            $models = VehicleBrandModels::where('id', '=', $model_id)->first();
        }

        $fuelType = array();
        if (isset($vehicle_details->fuel_type_id)) {
            $fuel_type_id = $vehicle_details->fuel_type_id;
            $fuelType = VehicleFuelType::where('id', '=', $fuel_type_id)->first();
        }

        $vehicle_type = array();
        if (isset($vehicle_details->vehicle_type_id)) {
            $vehicle_type_id = $vehicle_details->vehicle_type_id;
            $vehicle_type = VehicleTypes::where('id', '=', $vehicle_type_id)->first();
        }
        ...
        $profile_path = config('custom.upload.user.profile');
        return view('admin.modules.driver_cum_owner.show', [
            'user' => $user, 'user_role' => $this->user_role, 'plan' => $plan, 'plan_details' => $plan_details, 'earnings' => $earnings, 'customer_trip' => $customer_trip, 'driver_trip' => $driver_trip, 'money_spent' => $money_spent, 'referrals_done' => '0', 'trip_status' => 'Completed', 'current_location' => 'Ahm', 'subscription_status' => $subscription_status, 'vehicles' => $vehicles, 'cabs' => $cabs, 'drivers' => $drivers, 'trip' => $trip, 'brands' => $brands, 'models' => $models, 'fuelType' => $fuelType, 'referrals' => $referrals, 'vehicle_details' => $vehicle_details, 'vehicle_type' => $vehicle_type, 'driver_details' => $driver_details, 'banks' => $banks, 'last_updated_user_details' => $last_updated_user_details
        ]);
    }
