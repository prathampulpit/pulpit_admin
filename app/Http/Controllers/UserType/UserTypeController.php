...
class UserTypeController extends Controller
{
    ...
    public function index()
    {
        ...
        if (in_array("4", $role_id_arr) || $user_role == 'administrator') {
            return view('admin.modules.userType.index');
        } else {
            abort(403);
        }
    }
    ...
    public function index_json(Request $request)
    {
        ...
        if (request('per_page') == 'all') {
            ...
            $total = $this->userTypeRepository->getByParams($countcompany);
        } else {
            $total = request('per_page', config('custom.db.per_page', 100));
        }
        $params['per_page'] = $total;
        $users = $this->userTypeRepository->getPanelUsers($request, $params);
        return $users;
    }
    ...
    public function createEdit($panel, $id = null)
    {
        ...
        if (in_array("4", $role_id_arr) || $user_role == 'administrator') {
            ...
            if ($id) {
                ...
                $data = $this->userTypeRepository->getByParams($params);
            }

            return view('admin.modules.userType.store', [
                'data' => $data,
                'id' => $id,
                'user_role' => $user_role,
                'admin' => $admin
    ...
    public function store(Request $request)
    {
        ...
        $this->userTypeRepository->save($user);
        ...
        return redirect(route('admin.userType.index', ['panel' => Session::get('panel')]))->withMessage($message);
    }
    ...
    public function show($panel, $id)
    {
        ...
        $params['user_id'] = $id;
        $params['response_type'] = "single";
        $user = $this->userTypeRepository->getByParams($params);
    ...
    {
        $result = $this->userTypeRepository->toggleStatus($id);
        return (int) $result;
    }

    public function toggleReferalStatus($panel, $id)
    {
        $result = $this->userTypeRepository->toggleReferalStatus($id);
        return (int) $result;
    }

    public function destroy($panel, $id)
    {
        $result = $this->userTypeRepository->updateStatus($id);
        return (int) $result;
    }
}
