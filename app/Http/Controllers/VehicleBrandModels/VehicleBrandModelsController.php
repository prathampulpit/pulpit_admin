...
namespace App\Http\Controllers\VehicleBrandModels;
...
class VehicleBrandModelsController extends Controller
{
    ...
    public function index()
    {
        ...
        if (in_array("5", $role_id_arr) || $user_role == 'administrator') {
            return view('admin.modules.vehicleBrandModels.index');
        } else {
            abort(403);
        }
    }
    ...
    public function index_json(Request $request)
    {
        ...
        if (request('per_page') == 'all') {
            ...
            $total = $this->vehicleBrandModelsRepository->getByParams($countcompany);
        } else {
            $total = request('per_page', config('custom.db.per_page', 100));
        }
        $params['per_page'] = $total;
        $users = $this->vehicleBrandModelsRepository->getPanelUsers($request, $params);
        return $users;
    }
    ...
    public function createEdit($panel, $id = null)
    {
        ...
        if (in_array("5", $role_id_arr) || $user_role == 'administrator') {
            ...
            if ($id) {
                ...
                $data = $this->vehicleBrandModelsRepository->getByParams($params);
            }
            ...
            exit; */
            return view('admin.modules.vehicleBrandModels.store', [
                'data' => $data,
                'id' => $id,
                'user_role' => $user_role,
                'admin' => $admin,
    ...
    public function store(Request $request)
    {
        ...
        $this->vehicleBrandModelsRepository->save($user);
        ...
        return redirect(route('admin.vehicleBrandModels.index', ['panel' => Session::get('panel')]))->withMessage($message);
    }
    ...
    public function show($panel, $id)
    {
        ...
        $params['user_id'] = $id;
        $params['response_type'] = "single";
        $user = $this->vehicleBrandModelsRepository->getByParams($params);
    ...
    {
        $result = $this->vehicleBrandModelsRepository->toggleStatus($id);
        return (int) $result;
    }

    public function toggleReferalStatus($panel, $id)
    {
        $result = $this->vehicleBrandModelsRepository->toggleReferalStatus($id);
        return (int) $result;
    }

    public function destroy($panel, $id)
    {
        $result = $this->vehicleBrandModelsRepository->updateStatus($id);
        return (int) $result;
    }
}
