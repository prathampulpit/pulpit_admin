...
use App\Repositories\OfflineCustomerRepository;
...
{
    protected $offlineCustomerRepository;

    public function __construct(
        OfflineCustomerRepository $offlineCustomerRepository
    ) {
    ...
    public function index()
    {
        ...
        if (in_array("5", $role_id_arr) || $user_role == 'administrator') {
            return view('admin.modules.offlineCustomer.index');
        } else {
            abort(403);
        }
    }
    ...
    public function index_json(Request $request)
    {
        ...
        if (request('per_page') == 'all') {
            ...
            $total = $this->offlineCustomerRepository->getByParams($countcompany);
        } else {
            $total = request('per_page', config('custom.db.per_page', 100));
        }
        $params['per_page'] = $total;
        $users = $this->offlineCustomerRepository->getPanelUsers($request, $params);
        return $users;
    }
    ...
    public function createEdit($panel, $id = null)
    {
        ...
        if (in_array("5", $role_id_arr) || $user_role == 'administrator') {
            ...
            if ($id) {
                ...
                $data = $this->offlineCustomerRepository->getByParams($params);
            }

            return view('admin.modules.offlineCustomer.store', [
                'data' => $data,
                'id' => $id,
                'user_role' => $user_role,
                'admin' => $admin
    ...
    public function store(Request $request)
    {
        ...
        if ($request->has('logo')) {
            ...
            $path = $this->offlineCustomerRepository->upload($file, $uploadPath);
            $user['logo'] = $path; */
        ...
        $this->offlineCustomerRepository->save($user);
        ...
        return redirect(route('admin.offlineCustomer.index', ['panel' => Session::get('panel')]))->withMessage($message);
    }
    ...
    public function show($panel, $id)
    {
        ...
        $params['user_id'] = $id;
        $params['response_type'] = "single";
        $user = $this->offlineCustomerRepository->getByParams($params);
    ...
    {
        $result = $this->offlineCustomerRepository->toggleStatus($id);
        return (int) $result;
    }

    public function toggleReferalStatus($panel, $id)
    {
        $result = $this->offlineCustomerRepository->toggleReferalStatus($id);
        return (int) $result;
    }

    public function destroy($panel, $id)
    {
        $result = $this->offlineCustomerRepository->updateStatus($id);
        return (int) $result;
    }
}
