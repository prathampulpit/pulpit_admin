...
namespace App\Http\Controllers\SubscriptionCoupons;
...
use App\Repositories\SubscriptionCouponsRepository;
...
class SubscriptionCouponsController extends Controller
{
    protected $subscriptionCouponsRepository;

    public function __construct(SubscriptionCouponsRepository $subscriptionCouponsRepository) 
    {
        $this->subscriptionCouponsRepository = $subscriptionCouponsRepository;
    }
    ...
    public function index()
    {
        ...
        if (in_array("5", $role_id_arr) || $user_role == 'administrator') {
            return view('admin.modules.subscriptionCoupons.index');
        } else {
            abort(403);
        }
    }
    ...
    public function index_json(Request $request)
    {
        ...
        if (request('per_page') == 'all') {
            ...
            $total = $this->subscriptionCouponsRepository->getByParams($countcompany);
        } else {
            $total = request('per_page', config('custom.db.per_page', 100));
        }
        $params['per_page'] = $total;
        $users = $this->subscriptionCouponsRepository->getPanelUsers($request, $params);
        return $users;
    }
    ...
    public function createEdit($panel, $id = null)
    {
        ...
        if (in_array("5", $role_id_arr) || $user_role == 'administrator') {
            ...
            if ($id) {
                ...
                $data = $this->subscriptionCouponsRepository->getByParams($params);
            }

            return view('admin.modules.subscriptionCoupons.store', [
                'data' => $data,
                'id' => $id,
                'user_role' => $user_role,
                'admin' => $admin
    ...
    public function store(Request $request)
    {
        ...
        $this->subscriptionCouponsRepository->save($user);
        ...
        return redirect(route('admin.subscriptionCoupons.index', ['panel' => Session::get('panel')]))->withMessage($message);
    }
    ...
    public function show($panel, $id)
    {
        ...
        $params['user_id'] = $id;
        $params['response_type'] = "single";
        $user = $this->subscriptionCouponsRepository->getByParams($params);
    ...
    {
        $result = $this->subscriptionCouponsRepository->toggleStatus($id);
        return (int) $result;
    }

    public function toggleReferalStatus($panel, $id)
    {
        $result = $this->subscriptionCouponsRepository->toggleReferalStatus($id);
        return (int) $result;
    }

    public function destroy($panel, $id)
    {
        $result = $this->subscriptionCouponsRepository->updateStatus($id);
        return (int) $result;
    }
}
