...
class UserController extends Controller
{
    ...
    public function index()
    {
        ...
        if (in_array("4", $role_id_arr) || $user_role == 'administrator') {
            ...
            $today_joined = $this->UserRepository->getByParams($countcompany);
            ...
            $yesterday_joined = $this->UserRepository->getByParams($countcompany);
            ...
            $week_joined = $this->UserRepository->getByParams($countcompany);
            ...
            $this_month_joined = $this->UserRepository->getByParams($countcompany);
            ...
            $last_month_joined = $this->UserRepository->getByParams($countcompany);

            //$all_joined = User::count();
            $countcompany = array();
            $countcompany['count'] = true;
            $countcompany['type_id'] = '6';
            $all_joined = $this->UserRepository->getByParams($countcompany);
            ...
            $active_users = $this->UserRepository->getByParams($countcompany);
            ...
            $inactive_users = $this->UserRepository->getByParams($countcompany);
            ...
            $paid_users = $this->UserRepository->getByParams($countcompany);
            ...
            $expired_users = $this->UserRepository->getByParams($countcompany);
    ...
    public function documentVerification(Request $request)
    {
        ...
        // dd($key);
        // $message = trans('notifications.logo_status');
        // dd($message);
        if($status == 1){
        ...
        $message_name = "notifications." . $key;
        $loginUser = auth()->user();
        $message = New UserHistory();
        $message->edit_user_id = $loginUser->id;
        // dd(6);
        $message->message =  trans($message_name) . " " . $status_check." By " .$loginUser->first_name ." ". $loginUser->last_name;
        $message->user_id = $request->user_id;
        $message->save();
        
        if($status_check == "Rejected"){
            $agent = UserWorkProfile::where('user_type_id', '=', '2')->where('profile_id', '=', $id)->first();
            $travel = UserWorkProfile::Where('user_type_id', '=', '3')->where('profile_id', '=', $id)->first();
            if (!empty($agent) || !empty($travel)) {
                if(!empty($agent)){
                    $work = $agent;
                }else{ 
                    $work = $travel;
                ...
                if (!empty($users)) {
                    ...
                    if (!empty($device_token)) {
                        $notificationText = $users->first_name . " " . $users->last_name . " your Profile has not been Approved , Please resubmit your " . trans($message_name) . " .";
        ...
        if ($type == 'agent') {
            ...
            // if($status == 0){
                ...
                // // dd(6);
                // $message->message = "Agent Logo Rejected By " .$user_id->first_name ." ". $user_id->last_name;
                // $message->user_id = $request->user_id;
                // $message->save();
            // }
            if ($pan_card_url_status == 1 && $adhar_card_url_status == 1 && $registration_document_url_status == 1 && $adhar_card_back_url_status == 1) {
        ...
        if (!empty($users) && $users->is_approved == 1) {

            $device_type = $users->device_type;
            $device_token = $users->device_token;
            if (!empty($device_token)) {
                $notificationText = $users->first_name . " " . $users->last_name ." Your Profile has been approved, We are Glad to Onboard you.";
                $r = $this->notificationsRepository->sendPuchNotification($device_type, $device_token, $notificationText, $totalNotifications = '0', $pushMessageText = "", $title = "Pulpit",);
            }
        }
        echo "success";
    ...
    public function changeImage(Request $request)
    {
        ...
        $message_name = "notifications." . $image_key_val;
        $loginUser = auth()->user();
        $message = New UserHistory();
        $message->edit_user_id = $loginUser->id;
        $message->message =  trans($message_name) . " Updated By " . $loginUser->first_name ." ". $loginUser->last_name;
        $message->user_id = $id;
        $message->save();
        ...
        if ($type == 'agent') {
            // dd($request->get('pk_id'));
            $update = AgentUsers::find($pk_id);
            // dd($request->has($image_key_val));
            if ($request->has($image_key_val)) {
                ...
                // $history->edit_user_id = $user_id->id;
                // $history->message = "Registration Document updated by ".$user_id->first_name ." ". $user_id->last_name;
                // $history->user_id = $id;
                // $history->save();
            ...
            if ($pk_key == 'pan_card_url_status' || $pk_key == 'adhar_card_url_status' || $pk_key == 'adhar_card_back_url_status') {
                ...
                // $history = New UserHistory();
                // $history->edit_user_id = $user_id->id;
                // $history->message = $pk_key . " updated by ".$user_id->first_name ." ". $user_id->last_name;
                // $history->user_id = $id;
                // $history->save();
            }
    ...
    public function deleteImage(Request $request)
    {
        ...
        $message = New UserHistory();
        $message->edit_user_id = $loginUser->id;
        $message->message =  trans($message_name) . " Deleted By " . $loginUser->first_name ." ". $loginUser->last_name;
        $message->user_id = $id;
        $message->save();
    ...
    public function updateApprovedRequest($id)
    {
        ...
        foreach ($users as $val) {
            ...
            $subscriptionPlan = UserPurchasedPlans::where('user_id', $val['id'])->first();
            $subscriptionPlanStatus = 0;

            if($subscriptionPlan){
                if($subscriptionPlan->status == 1)
                {
                    $subscriptionPlanStatus = 1;
                }else{
                    $subscriptionPlanStatus = 0;
                }
            }
            if ($user_type_id == '2') { //Agent
                ...
                if ($isAgentStatus == 1 && $isDriverApproved == 1 && $subscriptionPlanStatus == 1) {
                    $userUpdate = User::find($user_id);
                    $userUpdate->is_approved = 1;
                    $userUpdate->save();
                } else if ($isAgentStatus == 0 || $isDriverApproved == 0 || $subscriptionPlanStatus == 0) {
                    $userUpdate = User::find($user_id);
                ...
                if (!empty($vehicles)) {
                    ...
                    if ($rc_front_url_status == 1 && $rc_back_url_status == 1 ) {
                    ...
                    } else if ($rc_front_url_status == 0 || $rc_back_url_status == 0 ) {
                ...
                if ($isAgentApproved == 1 && $isTravelApproved == 1 && $isDriverApproved == 1 && $subscriptionPlanStatus == 1) {
                    $userUpdate = User::find($user_id);
                    $userUpdate->is_approved = 1;
                    $userUpdate->save();
                } else if ($isAgentApproved == 0 || $isTravelApproved == 0 || $isDriverApproved == 0 || $subscriptionPlanStatus == 0) {
                    $userUpdate = User::find($user_id);
                    $userUpdate->is_approved = 0;
                    $userUpdate->save();
                }
                ...
                if (!empty($work)) {
                    ...
                    if (!empty($drivers)) {
                        ...
                        if ($dl_front_url_status == 1 && $dl_back_url_status == 1 && $subscriptionPlanStatus == 1) {
                            $isDriverApproved = 1;
                        } elseif ($dl_front_url_status == 0 || $dl_back_url_status == 0 || $subscriptionPlanStatus == 0) {
                            $isDriverApproved = 0;
                        }
                ...
                if ($isDriverApproved == 1 && $isVehicleApproved == 1 && $subscriptionPlanStatus == 1) {
                    $userUpdate = User::find($user_id);
                    $userUpdate->is_approved = 1;
                    $userUpdate->save();
                } else if ($isDriverApproved == 0 || $isVehicleApproved == 0 || $subscriptionPlanStatus == 0) {
                    $userUpdate = User::find($user_id);
                    $userUpdate->is_approved = 0;
                    $userUpdate->save();
                }
