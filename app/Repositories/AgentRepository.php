...
class AgentRepository extends BaseRepository
{
    ...
    public function getByParams($params)
    {
        ...
        $users->leftJoin("agent_users", function ($join) {
            $join->on("user_work_profile.profile_id", "=", "agent_users.id")->where("user_type.id", "=", "2");
        });
        ...
        // });
        $users->leftJoin("drivers", function ($join) {
            $join->on("user_work_profile.profile_id", "=", "drivers.id")->whereIn("user_type.id", [2]);
        });
        ...
        if (isset($params['plan_id'])) {
            //$users->leftJoin('user_purchased_plans', 'users.id', '=', 'user_purchased_plans.user_id');
            $users->where('subscription_plans.id', '=', $params['plan_id']);
            //$users->groupBy('user_purchased_plans.id');
        }
    ...
    public function getPanelPartnerUsers($request, $params)
    {
        ...
        $query->select(
            ...
            'agent_users.all_document_verify',
            'users.is_approved'
        );
        ...
        $query->leftJoin('city', 'users.city_id', '=', 'city.id');
        $query->leftJoin('states', 'users.state', '=', 'states.id');

        $query->where('type', '=', 'user');
        $query->where('user_type.id', '=', '2');
    ...
    public function getOneById($id)
    {
        ...
        $users->select('users.email', 'users.user_interest_status', 'users.status', 'users.id', 'users.id as uid', 'user_profiles.id as pid', 'user_profiles.user_id', 'user_profiles.country_id', 'user_profiles.language_id', 'user_profiles.first_name', 'user_profiles.last_name', 'user_profiles.dob', 'user_profiles.gender', 'user_profiles.phone', 'user_profiles.description', 'user_profiles.city', 'user_profiles.state', 'user_profiles.address', 'user_profiles.zipcode', 'user_profiles.profile_picture', 'user_profiles.profile_picture_status', 'user_profiles.twitter_link', 'user_profiles.custom_profile_link');
