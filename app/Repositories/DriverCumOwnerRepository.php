...
class DriverCumOwnerRepository extends BaseRepository
{
    protected $user;
    ...
    public function getByParams($params)
    {
        ...
        /* $users->leftJoin("agent_users",function($join){
            $join->on("user_work_profile.profile_id","=","agent_users.id")->where("user_type.id","=","5");
        });
        $users->leftJoin("drivers",function($join){
            $join->on("user_work_profile.profile_id","=","drivers.id")->whereIn("user_type.id",[4,5]);
        }); */
        $users->leftJoin("agent_users", function ($join) {
            // dd($join->on("user_work_profile.profile_id", "=", "agent_users.id")->where("agent_users.status", 1)->where("user_type.id", "=", 5));
            $join->on("user_work_profile.profile_id", "=", "agent_users.id")->where("agent_users.status", 1)->where("user_type.id", "=", 5);
        });
        $users->leftJoin("drivers", function ($join) {
         
            $join->on("user_work_profile.profile_id", "=", "drivers.id")->where("drivers.status", 1)->whereIn("user_type.id", [5]);
        });
        ...
        $users->leftJoin("vehicle_driving_mapping", function ($join) {
            $join->on("vehicles.id", "=", "vehicle_driving_mapping.vehicle_id")->whereIn("user_type.id", [5, 3]);
        });
    ...
    public function getPanelUsers($request, $params)
    {
        ...
        $query->select(DB::raw("CONVERT(users.id, CHAR) as user_id"), 'users.id', 'users.mobile_number', 'users.emailid', 'users.user_status', 'users.first_name', 'users.last_name', DB::raw("CONVERT_TZ(users.created_at,'+00:00','+05:30') as created_at"), 'agent_users.travel_name', 'agent_users.owner_name', 'agent_users.office_no', 'agent_users.total_business_year', 'agent_users.logo', 'agent_users.pan_card', 'agent_users.adhar_card', 'users.bank_account_id', 'user_type.name as user_type_name', 'drivers.mobile_numebr as driver_mobile_numebr', 'drivers.first_name as driver_first_name', 'drivers.last_name as driver_last_name', 'drivers.adhar_card_no as driver_adhar_card_no', 'drivers.driving_licence_no as driving_licence_no', 'drivers.driving_licence_expiry_date', 'drivers.street_address', 'drivers.pincode', 'users.otp', 'users.is_otp', 'agent_users.logo_status as agent_logo_status', 'agent_users.pan_card_url_status as pan_card_url_status', 'agent_users.adhar_card_url_status as adhar_card_url_status', 'agent_users.registration_document_url_status as registration_document_url_status', 'bank_account.bank_document_url_status', 'drivers.dl_front_url_status', 'drivers.dl_back_url_status',  'drivers.d_pan_card_url_status', 'drivers.police_verification_url_status', 'drivers.d_adhar_card_url_status', 'users.profile_pic', 'vehicles.vehicle_number', 'vehicle_types.name as vehicle_type_name', 'users.availability_status', DB::raw("CONVERT_TZ(user_purchased_plans.end_datetime,'+00:00','+05:30') as end_datetime"), 'subscription_plans.name as subscription_name', 'drivers.adhar_card_back_url as d_adhar_card_back_url', 'drivers.d_adhar_card_url_status', 'vehicles.insurance_doc_url_status', 'vehicles.permit_doc_url_status', 'vehicles.fitness_doc_url_status', 'vehicles.puc_doc_url_status', 'vehicles.agreement_doc_url_status', 'drivers.d_adhar_card_back_url_status', 'agent_users.adhar_card_back_url', 'agent_users.adhar_card_back_url_status', DB::raw("CONVERT_TZ(user_purchased_plans.start_datetime,'+00:00','+05:30') as start_datetime"), 'vehicles.all_document_verify', 'users.is_approved');
    ...
    public function getPanelPartnerUsers($request, $params)
    {
        ...
        $query = $this->user::whereRaw('1=1');
        $query->select(DB::raw("CONVERT(users.id, CHAR) as user_id"), 'users.id', 'users.mobile_number', 'users.emailid', 'users.user_status', 'users.first_name', 'users.last_name', DB::raw("CONVERT_TZ(users.created_at,'+00:00','+05:30') as created_at"), 'agent_users.travel_name', 'agent_users.owner_name', 'agent_users.office_no', 'agent_users.total_business_year', 'agent_users.logo', 'agent_users.pan_card', 'agent_users.adhar_card', 'users.bank_account_id', 'user_type.name as user_type_name', 'drivers.mobile_numebr as driver_mobile_numebr', 'drivers.first_name as driver_first_name', 'drivers.last_name as driver_last_name', 'drivers.adhar_card_no as driver_adhar_card_no', 'drivers.driving_licence_no as driving_licence_no', 'drivers.driving_licence_expiry_date', 'drivers.street_address', 'drivers.pincode', 'users.otp', 'users.is_otp', 'agent_users.logo_status as agent_logo_status', 'agent_users.pan_card_url_status as pan_card_url_status', 'agent_users.adhar_card_url_status as adhar_card_url_status', 'agent_users.registration_document_url_status as registration_document_url_status', 'bank_account.bank_document_url_status',  'drivers.dl_front_url_status', 'drivers.dl_back_url_status', 'drivers.police_verification_url_status', 'drivers.d_pan_card_url_status', 'drivers.d_adhar_card_url_status', 'users.profile_pic', 'vehicles.vehicle_number', 'vehicle_types.name as vehicle_type_name', 'users.availability_status', DB::raw("CONVERT_TZ(user_purchased_plans.end_datetime,'+00:00','+05:30') as end_datetime"), 'subscription_plans.name as subscription_name', 'vehicles.insurance_doc_url_status', 'vehicles.permit_doc_url_status', 'vehicles.fitness_doc_url_status', 'vehicles.puc_doc_url_status', 'vehicles.agreement_doc_url_status', 'drivers.d_adhar_card_back_url_status', 'agent_users.adhar_card_back_url', 'agent_users.adhar_card_back_url_status', 'users.is_approved');
        ...
        if ($request->get('search')) {
            $search = '%' . $request->get('search') . '%';
            //$query->where(function ($query) use ($search) {
            $query->whereRaw("(users.first_name like " . "'" . $search . "' OR users.last_name like " . "'" . $search . "' OR vehicles.vehicle_number like " . "'" . $search . "' OR users.mobile_number like " . "'" . $search . "') ");
            //});
        }
    ...
    public function getByParamsForCount($params)
    {
        $users = $this->user::whereRaw('1=1');
        $users->select(DB::raw("CONVERT(users.id, CHAR) as user_id"), 'users.id', 'agent_users.id as agent_id', 'drivers.id as driver_id', 'users.mobile_number', 'users.emailid', 'users.user_status', 'users.first_name', 'users.last_name', DB::raw("CONVERT_TZ(users.created_at,'+00:00','+05:30') as created_at"), 'agent_users.travel_name', 'agent_users.owner_name', 'agent_users.office_no', 'agent_users.total_business_year', 'agent_users.logo', 'agent_users.pan_card', 'agent_users.adhar_card', 'users.bank_account_id', 'user_type.name as user_type_name', 'drivers.mobile_numebr as driver_mobile_numebr', 'drivers.first_name as driver_first_name', 'drivers.last_name as driver_last_name', 'drivers.adhar_card_no as driver_adhar_card_no', 'drivers.driving_licence_no as driving_licence_no', 'drivers.driving_licence_expiry_date', 'drivers.street_address', 'drivers.pincode', 'drivers.pincode', 'agent_users.pan_card_url', 'agent_users.adhar_card_url', 'agent_users.registration_document_url', 'drivers.adhar_card_url as d_adhar_card_url', 'drivers.pan_card_url as d_pan_card_url', 'drivers.dl_front_url', 'drivers.dl_back_url', 'drivers.police_verification_url', 'agent_users.logo as agent_logo', 'bank_account.document_url as bank_document_url', 'user_work_profile.user_type_id', 'user_work_profile.profile_id', 'agent_users.logo_status as agent_logo_status', 'agent_users.pan_card_url_status as pan_card_url_status', 'agent_users.adhar_card_url_status as adhar_card_url_status', 'agent_users.registration_document_url_status as registration_document_url_status', 'bank_account.bank_document_url_status', 'drivers.dl_front_url_status', 'drivers.dl_back_url_status', 'drivers.d_pan_card_url_status', 'drivers.police_verification_url_status', 'drivers.d_adhar_card_url_status', 'users.state', 'users.city_id', 'users.otp', 'users.is_otp', 'users.profile_pic', 'users.reference_code', 'drivers.pan_card_number', 'bank_account.bank_name', 'bank_account.branch_name', 'bank_account.account_number as bank_account_number', 'vehicles.vehicle_number', 'vehicle_types.name as vehicle_type_name', 'users.availability_status', DB::raw("CONVERT_TZ(user_purchased_plans.end_datetime,'+00:00','+05:30') as end_datetime"), 'subscription_plans.name as subscription_name', 'vehicles.brand_id', 'vehicles.model_id', 'vehicles.fuel_type_id', 'vehicles.registration_year', 'vehicles.insurance_doc_url', 'vehicles.permit_doc_url', 'vehicles.fitness_doc_url', 'vehicles.puc_doc_url', 'vehicles.agreement_doc_url', 'vehicles.id as vehicle_id', 'vehicles.insurance_doc_url_status', 'vehicles.permit_doc_url_status', 'vehicles.fitness_doc_url_status', 'vehicles.puc_doc_url_status', 'vehicles.agreement_doc_url_status', 'vehicles.insurance_exp_date', 'vehicles.permit_exp_date', 'vehicles.fitness_exp_date', 'vehicles.puc_exp_date', 'drivers.adhar_card_back_url', 'drivers.d_adhar_card_back_url_status', 'users.is_approved');
        $users->leftJoin("user_work_profile", function ($join) {
            $join->on("users.id", "=", "user_work_profile.user_id")->where("user_work_profile.status", 1);
        });

        $users->leftJoin('user_type', 'user_work_profile.user_type_id', '=', 'user_type.id');
        $users->leftJoin('user_bank_mapping', 'users.id', '=', 'user_bank_mapping.user_id');
        $users->leftJoin('bank_account', 'user_bank_mapping.bank_account_id', '=', 'bank_account.id');
        //$users->leftJoin('agent_users', 'user_type.id', '=', 'agent_users.user_type_id');
        $users->leftJoin("agent_users", function ($join) {
        ...
        if (isset($params['plan_id']) && !empty($params['plan_id'])) {
            if ($params['plan_id'] == 'all') {
                $users->where('subscription_plans.name', '!=', '');
            } else {
                $users->where('user_purchased_plans.subscription_plan_id', '=', $params['plan_id']);
            }
        } else {
            if (isset($params['record_type']) && $params['record_type'] == 'paid_user') {
                $users->where('subscription_plans.name', '!=', '');
            }
        }
