...
@section('title') Bubble Text @endsection
    ...
    <div class="two-col">
        ...
        <div class="right-sec" v-cloak>
            ...
            <div class="top-bar d-flex align-items-center">
                <h2>{{ @trans('bubble_text_messages.cms') }}</h2>
                ...
                <div class="right-table-container ml-auto">
                    ...
                    <a href="{{ route('admin.bubble_text_messages.create',['panel' => Session::get('panel')]) }}"
                        class="btn btn-em btn-primary-em">{{ @trans('bubble_text_messages.add_cms') }}</a>
                </div>
            </div>
                    ...
                    <div class="table-responsive table-checkable">
                        <table class="table">
                            <thead>
                                <tr>
                                    <th>#</th>
                                    <th>{{ @trans('bubble_text_messages.bubble_text_en') }}<a href="javascript:void(0)"
                                            :class="classSort('name')" @click="sort('name')"></a>
                                    </th>
                                    <th>{{ @trans('bubble_text_messages.bubble_text_sw') }}<a href="javascript:void(0)"
                                            :class="classSort('name')" @click="sort('name')"></a>
                                    </th>
                                    <th>{{ @trans('bubble_text_messages.expiry_date') }}<a href="javascript:void(0)"
                                            :class="classSort('expiry_date')" @click="sort('expiry_date')"></a>
                                    </th>
                            ...
                            <tbody>
                                <tr v-for="(item,index) in items">
                                    <td>@{{ index + from }}</td>
                                    <td>@{{ item.bubble_text_en }}</td>
                                    <td>@{{ item.bubble_text_sw }}</td>
                                    <td>@{{ item.expiry_date }}</td>
                                    <td class="act-btn">
                                        <a :href="'{{url(Session::get('panel').'/bubble_text_messages/edit')}}/'+ item.id" title="Edit"
                                            class="btn btn-edit"><i class="material-icons">edit</i>
                                        </a>

                                        <a :href="'{{url(Session::get('panel').'/bubble_text_messages/show')}}/'+ item.id" title="View"
                                            class="btn btn-view">
...
<script>
    var indexUrlJson = "{{route('admin.bubble_text_messages.index_json',['panel' => Session::get('panel')])}}";
    var deleteUrl = "{{ url(Session::get('panel').'/bubble_text_messages/destroy') }}";
...
<script type="text/javascript" src="{{ asset('js/admin/bubble_text_messages.js') }}"></script>
@endpush