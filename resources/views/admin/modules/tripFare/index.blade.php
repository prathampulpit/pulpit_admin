            ...
            <div class="page-header">
                ...
                <div class="d-flex">
                    <div class="justify-content-center">
                        ...
                        <a class="btn btn-primary my-2 btn-icon-text" href="{{ route('admin.tripFare.create',['panel' => Session::get('panel')]) }}">
                            <i class="fe fe-plus mr-2"></i> {{ @trans('tripFare.add_tripFare') }} </a>
                    </div>
                </div>
            </div>
            <!-- End Page Header -->
                                            ...
                                            <table id="example" class="table table-striped table-bordered text-nowrap dataTables_wrapper dt-bootstrap4 no-footer">
                                                <thead>
                                                    <tr>
                                                        ...
                                                        <th><b>Area Name</b><a href="javascript:void(0)"
                                                                :class="classSort('area_name')" @click="sort('area_name')"></a></th>
                                                        <th><b>{{ @trans('tripFare.city') }}</b><a href="javascript:void(0)"
                                                        :class="classSort('city_id')" @click="sort('city_id')"></a></th>
                                                        <th><b>{{ @trans('tripFare.vehicle') }}</b><a href="javascript:void(0)"
                                                                :class="classSort('vehicle_type_id')" @click="sort('vehicle_type_id')"></a></th>
                                                        <th><b>{{ @trans('tripFare.base_fare') }}</b><a href="javascript:void(0)"
                                                                :class="classSort('base_fare')" @click="sort('base_fare')"></a>
                                                        </th>
                                                        <th><b>{{ @trans('tripFare.minimum_fare') }}</b><a href="javascript:void(0)"
                                                                :class="classSort('minimum_fare')" @click="sort('minimum_fare')"></a>
                                                        </th>
                                                ...
                                                <tbody>
                                                    <tr v-for="(item,index) in items">
                                                        ...
                                                        <td>@{{ item.minimum_fare }}</td>
                                                        <td>@{{ item.gst }}</td>
                                                        <td class="act-btn">                                
                                                            <a :href="'{{url(Session::get('panel').'/tripFare/show')}}/'+ item.id" title="View"
                                                                class="btn-view btn-sm">
                                                                <i class="material-icons">remove_red_eye</i>
                                                            </a>                                        
                                                            <a :href="'{{url(Session::get('panel').'/tripFare/edit')}}/'+ item.id" title="Edit"
...
<script>
    var user_indexUrlJson = "{{route('admin.tripFare.index_json',['panel' => Session::get('panel')])}}";
    var toggle_status_url = "{{url(Session::get('panel').'/users/toggle-status')}}";
    var user_deleteUrl = "{{route('admin.tripFare.destroy',['panel' => Session::get('panel')])}}";
