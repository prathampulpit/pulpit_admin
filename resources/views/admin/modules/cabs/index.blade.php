    ...
    <div class="container-fluid">
        <div class="inner-body">
            ...
            <div class="page-header">
                <div>
                    <h2 class="main-content-title tx-24 mg-b-5">{{ @trans('cabs.cabs') }}</h2>
                    <!-- <ol class="breadcrumb">
                ...
                <div class="d-flex">
                    <div class="justify-content-center">
                        ...
                        <!-- <a class="btn btn-primary my-2 btn-icon-text" href="{{ route('admin.cabs.create',['panel' => Session::get('panel')]) }}">
                            <i class="fe fe-plus mr-2"></i> {{ @trans('cabs.add_cabs') }} </a> -->
                    </div>
                </div>
            </div>
            <!-- End Page Header -->
                                        ...
                                        <div class="table-checkable table-scrollable" style="height: 500px; overflow-x: scroll;">
                                            <table id="example" class="table table-striped table-bordered text-nowrap dataTables_wrapper dt-bootstrap4 no-footer">
                                                <thead>
                                                    <tr>
                                                        <th>#</th>
                                                        <th><b>{{ @trans('cabs.username') }}</b><a href="javascript:void(0)"
                                                                :class="classSort('user_id')" @click="sort('user_id')"></a>
                                                        </th>
                                                        <th><b>{{ @trans('cabs.vehicle') }}</b><a href="javascript:void(0)"
                                                                :class="classSort('vehicle_id')" @click="sort('vehicle_id')"></a>
                                                        </th>
                                                        <th><b>{{ @trans('cabs.cab_post_type') }}</b><a href="javascript:void(0)"
                                                                :class="classSort('cab_post_type')" @click="sort('cab_post_type')"></a>
                                                        <th><b>{{ @trans('cabs.start_date') }}</b><a href="javascript:void(0)"
                                                                :class="classSort('start_date')" @click="sort('start_date')"></a>
                                                        </th>
                                                        <th><b>{{ @trans('cabs.start_time') }}</b><a href="javascript:void(0)"
                                                                :class="classSort('start_time')" @click="sort('start_time')"></a>
                                                        </th>
                                                        <th><b>{{ @trans('cabs.available_for') }}</b><a href="javascript:void(0)"
                                                                :class="classSort('available_for')" @click="sort('available_for')"></a>
                                                        </th>
                                                        <th><b>{{ @trans('cabs.action') }}</b></th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    <tr v-for="(item,index) in items">
                                                        <td>@{{ index + from }}</td>
                                                        <td><a :href="'{{url(Session::get('panel').'/cabs/show')}}/'+ item.id">@{{ item.first_name }} @{{ item.last_name }}</a></td>
                                                        <td><a :href="'{{url(Session::get('panel').'/cabs/show')}}/'+ item.id">@{{ item.vehicle_number }}</a></td>
                                                        <td><a :href="'{{url(Session::get('panel').'/cabs/show')}}/'+ item.id">@{{ item.cab_post_type }}</a></td>
                                                        <td>@{{ item.start_date }}</td>
                                                        <td>@{{ item.start_time }}</td>
                                                        <td>@{{ item.available_for }}</td>
                                                        <td class="act-btn">                                
                                                            <a :href="'{{url(Session::get('panel').'/cabs/show')}}/'+ item.id" title="View"
                                                                class="btn-view btn-sm">
                                                                <i class="material-icons">remove_red_eye</i>
                                                            </a>                                        
                                                            <!-- <a :href="'{{url(Session::get('panel').'/cabs/edit')}}/'+ item.id" title="Edit"
...
<script>
    var user_indexUrlJson = "{{route('admin.cabs.index_json',['panel' => Session::get('panel')])}}";
    var toggle_status_url = "{{url(Session::get('panel').'/users/toggle-status')}}";
    var user_deleteUrl = "{{route('admin.cabs.destroy',['panel' => Session::get('panel')])}}";
...
<script type="text/javascript" src="{{ asset('js/admin/cabs.js') }}"></script>
@endpush