    ...
    <div class="container-fluid">
        <div class="inner-body">
            ...
            <div class="page-header">
                <div>
                    <h2 class="main-content-title tx-24 mg-b-5">{{ @trans('tripFare.outstation') }} {{ @trans('tripFare.tripFare') }} - {{ @trans('tripFare.website') }}</h2>
                    <!-- <ol class="breadcrumb">
                ...
                <div class="d-flex">
                    <div class="justify-content-center">
                        ...
                        <a class="btn btn-primary my-2 btn-icon-text" href="{{ route('admin.websiteOutStationTripFare.create',['panel' => Session::get('panel')]) }}">
                            <i class="fe fe-plus mr-2"></i>{{ @trans('tripFare.add') }} {{ @trans('tripFare.outstation') }} {{ @trans('tripFare.tripFare') }} </a>
                    </div>
                </div>
            </div>
            <!-- End Page Header -->
                                            ...
                                            <table id="example" class="table table-striped table-bordered text-nowrap dataTables_wrapper dt-bootstrap4 no-footer">
                                                <thead>
                                                    <tr>
                                                        ...
                                                        <th><b>Driver Allowances</b><a href="javascript:void(0)"
                                                                                                              :class="classSort('driver_allowances')" @click="sort('driver_allowances')"></a>
                                                        </th>
                                                        <th><b>{{ @trans('tripFare.per_km') }}</b></th>
                                                ...
                                                <tbody>
                                                    <tr v-for="(item,index) in items">
                                                        ...
                                                        <td>@{{ item.driver_allowances }}</td>
                                                        <td>@{{ item.per_km }}</td>
                                                        <td class="act-btn">                                
                                                            <a :href="'{{url(Session::get('panel').'/websiteOutStationTripFare/show')}}/'+ item.id" title="View"
                                                               class="btn-view btn-sm">
                                                                <i class="material-icons">remove_red_eye</i>
                                                            </a>                                        
                                                            <a :href="'{{url(Session::get('panel').'/websiteOutStationTripFare/edit')}}/'+ item.id" title="Edit"
...
<script>
    var user_indexUrlJson = "{{route('admin.websiteOutStationTripFare.index_json',['panel' => Session::get('panel')])}}";
    var toggle_status_url = "{{url(Session::get('panel').'/users/toggle-status')}}";
    var user_deleteUrl = "{{route('admin.websiteOutStationTripFare.destroy',['panel' => Session::get('panel')])}}";
...
<script type="text/javascript" src="{{ asset('js/admin/websiteOutStationTripFare.js') }}"></script>
@endpush