            ...
            <div class="page-header">
                ...
                <div class="d-flex">
                    <div class="justify-content-center">
                        <a class="btn btn-primary my-2 btn-icon-text" href="{{ route('admin.vehicleTypes.create',['panel' => Session::get('panel')]) }}">
                            <i class="fe fe-plus mr-2"></i> {{ @trans('vehicleTypes.add_vehicleTypes') }} </a>
                    </div>
                </div>
            </div>
            <!-- End Page Header -->
                                        ...
                                        <div class="table-checkable">
                                            <table id="example" class="table table-striped table-bordered text-nowrap dataTables_wrapper dt-bootstrap4 no-footer">
                                                <thead>
                                                    <tr>
                                                        <th>#</th>
                                                        <th><b>{{ @trans('vehicleTypes.image') }}</b><a href="javascript:void(0)"
                                                                :class="classSort('img')" @click="sort('img')"></a>
                                                        </th>
                                                        <th><b>{{ @trans('vehicleTypes.name') }}</b><a href="javascript:void(0)"
                                                                :class="classSort('name')" @click="sort('name')"></a>
                                                        </th>
                                                        <th><b>{{ @trans('vehicleTypes.action') }}</b></th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    <tr v-for="(item,index) in items">
                                                        <td>@{{ index + from }}</td>
                                                        <td><img v-bind:src="'{{env('S3_BUCKET_URL')}}/' + item.img" width='50'/></td>
                                                        <td>@{{ item.name }}</td>
                                                        <td class="act-btn">                                
                                                            <!-- <a :href="'{{url(Session::get('panel').'/vehicleTypes/show')}}/'+ item.id" title="View"
                                                                class="btn-sm btn-view">
                                                                <i class="material-icons">remove_red_eye</i>
                                                            </a>  -->                                       
                                                            <a :href="'{{url(Session::get('panel').'/vehicleTypes/edit')}}/'+ item.id" title="Edit"
...
<script>
    var user_indexUrlJson = "{{route('admin.vehicleTypes.index_json',['panel' => Session::get('panel')])}}";
    var toggle_status_url = "{{url(Session::get('panel').'/users/toggle-status')}}";
    var user_deleteUrl = "{{route('admin.vehicleTypes.destroy',['panel' => Session::get('panel')])}}";
...
<script type="text/javascript" src="{{ asset('js/admin/vehicleTypes.js') }}"></script>
@endpush