            ...
            <div class="page-header">
                ...
                <div class="d-flex">
                    <div class="justify-content-center">
                        ...
                        <a class="btn btn-primary my-2 btn-icon-text" href="{{ route('admin.subscriptionPlans.create',['panel' => Session::get('panel')]) }}">
                            <i class="fe fe-plus mr-2"></i> {{ @trans('subscriptionPlans.add_subscriptionPlans') }} </a>
                    </div>
                </div>
            </div>
            <!-- End Page Header -->
                                        ...
                                        <div class="table-checkable">
                                            <table id="example" class="table table-striped table-bordered text-nowrap dataTables_wrapper dt-bootstrap4 no-footer">
                                                <thead>
                                                    <tr>
                                                        <th>#</th>
                                                        <th><b>{{ @trans('subscriptionPlans.name') }}</b><a href="javascript:void(0)"
                                                                :class="classSort('name')" @click="sort('name')"></a>
                                                        </th>
                                                        <th><b>Type</b><a href="javascript:void(0)"
                                                                :class="classSort('is_agent')" @click="sort('is_agent')"></a>
                                                        </th>
                                                        <th><b>{{ @trans('subscriptionPlans.vehicleType') }}</b><a href="javascript:void(0)"
                                                                :class="classSort('vehicle_type_id')" @click="sort('vehicle_type_id')"></a>
                                                        </th>
                                                        <th><b>{{ @trans('subscriptionPlans.plan_validity') }}</b><a href="javascript:void(0)"
                                                                :class="classSort('plan_validity')" @click="sort('plan_validity')"></a>
                                                        </th>
                                                        <th><b>{{ @trans('subscriptionPlans.price') }}</b><a href="javascript:void(0)"
                                                                :class="classSort('price')" @click="sort('price')"></a>
                                                        </th>
                                                        <th><b>{{ @trans('subscriptionPlans.action') }}</b></th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    <tr v-for="(item,index) in items">
                                                        <td>@{{ index + from }}</td>
                                                        <td>@{{ item.name }}</td>
                                                        <td v-if="item.is_agent=='1'">Agent</td>
                                                        <td v-else="item.is_agent=='0'">Vehicle</td>
                                                        <td>@{{ item.vehicle_types_name }}</td>
                                                        <td>@{{ item.plan_validity }}</td>
                                                        <td>₹@{{ item.price }}</td>
                                                        <td class="act-btn">                                
                                                            <!-- <a :href="'{{url(Session::get('panel').'/subscriptionPlans/show')}}/'+ item.id" title="View"
                                                                class="btn-view btn-sm">
                                                                <i class="material-icons">remove_red_eye</i>
                                                            </a> -->                                        
                                                            <a :href="'{{url(Session::get('panel').'/subscriptionPlans/edit')}}/'+ item.id" title="Edit"
...
<script>
    var user_indexUrlJson = "{{route('admin.subscriptionPlans.index_json',['panel' => Session::get('panel')])}}";
    var toggle_status_url = "{{url(Session::get('panel').'/users/toggle-status')}}";
    var user_deleteUrl = "{{route('admin.subscriptionPlans.destroy',['panel' => Session::get('panel')])}}";
...
<script type="text/javascript" src="{{ asset('js/admin/subscriptionPlans.js') }}"></script>
@endpush