...
@section('title') Vehicle Colour @endsection
            ...
            <div class="page-header">
                ...
                <div class="d-flex">
                    <div class="justify-content-center">
                        ...
                        <a class="btn btn-primary my-2 btn-icon-text" href="{{ route('admin.vehicleColour.create',['panel' => Session::get('panel')]) }}">
                            <i class="fe fe-plus mr-2"></i> {{ @trans('vehicleColour.add_vehicleColour') }} </a>
                    </div>
                </div>
            </div>
            <!-- End Page Header -->
                                        ...
                                        <div class="table-checkable">
                                            <table id="example" class="table table-striped table-bordered text-nowrap dataTables_wrapper dt-bootstrap4 no-footer">
                                                <thead>
                                                    <tr>
                                                        <th>#</th>
                                                        <!-- <th><b>{{ @trans('vehicleBrandModels.model_name') }}</b><a href="javascript:void(0)"
                                                                :class="classSort('model_name')" @click="sort('model_name')"></a>
                                                        </th> -->
                                                        <th><b>{{ @trans('vehicleColour.name') }}</b><a href="javascript:void(0)"
                                                                :class="classSort('name')" @click="sort('name')"></a>
                                                        </th>
                                                        <th><b>{{ @trans('vehicleColour.colour_code') }}</b><a href="javascript:void(0)"
                                                                :class="classSort('colour_code')" @click="sort('colour_code')"></a>
                                                        </th>
                                                        <th><b>{{ @trans('vehicleColour.action') }}</b></th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    <tr v-for="(item,index) in items">
                                                        ...
                                                        <td class="act-btn">                                
                                                            <!-- <a :href="'{{url(Session::get('panel').'/vehicleColour/show')}}/'+ item.id" title="View"
                                                                class="btn-sm btn-view">
                                                                <i class="material-icons">remove_red_eye</i>
                                                            </a>  -->                                       
                                                            <a :href="'{{url(Session::get('panel').'/vehicleColour/edit')}}/'+ item.id" title="Edit"
...
<script>
    var user_indexUrlJson = "{{route('admin.vehicleColour.index_json',['panel' => Session::get('panel')])}}";
    var toggle_status_url = "{{url(Session::get('panel').'/users/toggle-status')}}";
    var user_deleteUrl = "{{route('admin.vehicleColour.destroy',['panel' => Session::get('panel')])}}";
...
<script type="text/javascript" src="{{ asset('js/admin/vehicleColour.js') }}"></script>
@endpush