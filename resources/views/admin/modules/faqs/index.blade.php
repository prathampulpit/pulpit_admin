            ...
            <div class="page-header">
                ...
                <div class="d-flex">
                    <div class="justify-content-center">
                        ...
                        <a class="btn btn-primary my-2 btn-icon-text" href="{{ route('admin.faqs.create',['panel' => Session::get('panel')]) }}">
                            <i class="fe fe-plus mr-2"></i> {{ @trans('faqs.add_faqs') }} </a>
                    </div>
                </div>
            </div>
            <!-- End Page Header -->
                                        ...
                                        <div class="table-checkable">
                                            <table id="example" class="table table-striped table-bordered text-nowrap dataTables_wrapper dt-bootstrap4 no-footer">
                                                <thead>
                                                    <tr>
                                                        <th>#</th>
                                                        <th><b>{{ @trans('faqs.question') }}</b><a href="javascript:void(0)"
                                                                :class="classSort('question')" @click="sort('question')"></a>
                                                        <th><b>{{ @trans('faqs.answer') }}</b><a href="javascript:void(0)"
                                                                :class="classSort('answer')" @click="sort('answer')"></a>
                                                        </th>
                                                        <th><b>{{ @trans('faqs.action') }}</b></th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    <tr v-for="(item,index) in items">
                                                        ...
                                                        <td class="act-btn">                                
                                                            <!-- <a :href="'{{url(Session::get('panel').'/faqs/show')}}/'+ item.id" title="View"
                                                                class="btn-view btn-sm">
                                                                <i class="material-icons">remove_red_eye</i>
                                                            </a> -->                                        
                                                            <a :href="'{{url(Session::get('panel').'/faqs/edit')}}/'+ item.id" title="Edit"
...
<script>
    var user_indexUrlJson = "{{route('admin.faqs.index_json',['panel' => Session::get('panel')])}}";
    var toggle_status_url = "{{url(Session::get('panel').'/users/toggle-status')}}";
    var user_deleteUrl = "{{route('admin.faqs.destroy',['panel' => Session::get('panel')])}}";
...
<script type="text/javascript" src="{{ asset('js/admin/faqs.js') }}"></script>
@endpush