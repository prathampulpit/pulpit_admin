...
@section('title') Vehicle Brands @endsection
            ...
            <div class="page-header">
                ...
                <div class="d-flex">
                    <div class="justify-content-center">
                        ...
                        <a class="btn btn-primary my-2 btn-icon-text" href="{{ route('admin.vehicleBrands.create',['panel' => Session::get('panel')]) }}">
                            <i class="fe fe-plus mr-2"></i> {{ @trans('vehicleBrands.add_vehicleBrands') }} </a>
                    </div>
                </div>
            </div>
            <!-- End Page Header -->
                                        ...
                                        <div class="table-checkable">
                                            <table id="example" class="table table-striped table-bordered text-nowrap dataTables_wrapper dt-bootstrap4 no-footer">
                                                <thead>
                                                    <tr>
                                                        <th>#</th>
                                                        <th><b>{{ @trans('vehicleBrands.logo') }}</b><a href="javascript:void(0)"
                                                                :class="classSort('logo')" @click="sort('logo')"></a>
                                                        <th><b>{{ @trans('vehicleBrands.name') }}</b><a href="javascript:void(0)"
                                                                :class="classSort('name')" @click="sort('name')"></a>
                                                        </th>
                                                        <th><b>{{ @trans('vehicleBrands.action') }}</b></th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    <tr v-for="(item,index) in items">
                                                        <td>@{{ index + from }}</td>
                                                        <td><img v-bind:src="'{{env('S3_BUCKET_URL')}}/' + item.logo" width='50'/></td>
                                                        <td>@{{ item.name }}</td>
                                                        <td class="act-btn">                                
                                                            <!-- <a :href="'{{url(Session::get('panel').'/vehicleBrands/show')}}/'+ item.id" title="View"
                                                                class="btn-view btn-sm">
                                                                <i class="material-icons">remove_red_eye</i>
                                                            </a> -->                                        
                                                            <a :href="'{{url(Session::get('panel').'/vehicleBrands/edit')}}/'+ item.id" title="Edit"
...
<script>
    var user_indexUrlJson = "{{route('admin.vehicleBrands.index_json',['panel' => Session::get('panel')])}}";
    var toggle_status_url = "{{url(Session::get('panel').'/users/toggle-status')}}";
    var user_deleteUrl = "{{route('admin.vehicleBrands.destroy',['panel' => Session::get('panel')])}}";
...
<script type="text/javascript" src="{{ asset('js/admin/vehicleBrands.js') }}"></script>
@endpush