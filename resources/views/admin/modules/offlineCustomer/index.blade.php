...
@section('title') Offline Customer @endsection
    ...
    <div class="container-fluid">
        <div class="inner-body">
            ...
            <div class="page-header">
                <div>
                    <h2 class="main-content-title tx-24 mg-b-5">{{ @trans('offlineCustomer.offlineCustomer') }}</h2>
                    <!-- <ol class="breadcrumb">
                ...
                <div class="d-flex">
                    <div class="justify-content-center">
                        ...
                        <!-- <a class="btn btn-primary my-2 btn-icon-text" href="{{ route('admin.offlineCustomer.create',['panel' => Session::get('panel')]) }}">
                            <i class="fe fe-plus mr-2"></i> {{ @trans('offlineCustomer.add_offlineCustomer') }} </a> -->
                    </div>
                </div>
            </div>
            <!-- End Page Header -->
                                        ...
                                        <div class="table-checkable">
                                            <table id="example" class="table table-striped table-bordered text-nowrap dataTables_wrapper dt-bootstrap4 no-footer">
                                                <thead>
                                                    <tr>
                                                        <th>#</th>
                                                        <th><b>{{ @trans('offlineCustomer.name') }}</b><a href="javascript:void(0)"
                                                                :class="classSort('logo')" @click="sort('logo')"></a></th>
                                                        <th><b>{{ @trans('offlineCustomer.address_line1') }}</b><a href="javascript:void(0)"
                                                            :class="classSort('address_line1')" @click="sort('address_line1')"></a></th>
                                                        <th><b>{{ @trans('offlineCustomer.address_line2') }}</b><a href="javascript:void(0)"
                                                                :class="classSort('name')" @click="sort('address_line2')"></a></th>
                                                        <th><b>{{ @trans('offlineCustomer.mobile_number') }}</b><a href="javascript:void(0)"
                                                                :class="classSort('name')" @click="sort('mobile_number')"></a></th>
                                                        <!-- <th><b>{{ @trans('offlineCustomer.action') }}</b></th> -->
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    <tr v-for="(item,index) in items">
                                                        ...
                                                        <!-- <td class="act-btn">                                
                                                            <a :href="'{{url(Session::get('panel').'/offlineCustomer/show')}}/'+ item.id" title="View"
                                                                class="btn-view btn-sm">
                                                                <i class="material-icons">remove_red_eye</i>
                                                            </a>                                        
                                                            <a :href="'{{url(Session::get('panel').'/offlineCustomer/edit')}}/'+ item.id" title="Edit"
...
<script>
    var user_indexUrlJson = "{{route('admin.offlineCustomer.index_json',['panel' => Session::get('panel')])}}";
    var toggle_status_url = "{{url(Session::get('panel').'/users/toggle-status')}}";
    var user_deleteUrl = "{{route('admin.offlineCustomer.destroy',['panel' => Session::get('panel')])}}";
...
<script type="text/javascript" src="{{ asset('js/admin/offlineCustomer.js') }}"></script>
@endpush