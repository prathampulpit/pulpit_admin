...
@section('title') Send Money Labels @endsection
    ...
    <div class="two-col">
        ...
        <div class="right-sec" v-cloak>
            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="javascript:void(0)">{{ @trans('send_money_labels.send_money_label') }}</a></li>
                    <li class="breadcrumb-item active" aria-current="page">{{ @trans('app.list') }}</li>
                </ol>
            </nav>

            <div class="top-bar d-flex align-items-center">
                <h2>{{ @trans('send_money_labels.manage_send_money_label') }}</h2>
                    ...
                    <div class="table-responsive table-checkable">
                        <table class="table">
                            <thead>
                                <tr>
                                    <th>#</th>
                                    <th>{{ @trans('send_money_labels.icon') }}<a href="javascript:void(0)"
                                            :class="classSort('icon')" @click="sort('icon')"></a>
                                    </th>
                                    <th>{{ @trans('send_money_labels.display_name') }}<a href="javascript:void(0)"
                                            :class="classSort('display_name')" @click="sort('display_name')"></a>
                                    </th>
                                    <th>{{ @trans('send_money_labels.display_name_sw') }}<a href="javascript:void(0)"
                                            :class="classSort('display_name_sw')" @click="sort('display_name_sw')"></a>
                                    </th>
                                    <th>{{ @trans('send_money_labels.sub_title') }}<a href="javascript:void(0)"
                                            :class="classSort('sub_title')" @click="sort('sub_title')"></a>
                                    </th>
                                    <th>{{ @trans('send_money_labels.sub_title_sw') }}<a href="javascript:void(0)"
                                            :class="classSort('sub_title_sw')" @click="sort('sub_title_sw')"></a>
                                    </th>
                                    <th>{{ @trans('send_money_labels.code') }}<a href="javascript:void(0)"
                                            :class="classSort('code')" @click="sort('code')"></a>
                                    </th>
                                    <th>{{ @trans('send_money_labels.status') }}</th>
                            ...
                            <tbody>
                                <tr v-for="(item,index) in items">
                                    <td>@{{ index + from }}</td>
                                    <td><img v-bind:src="'../storage/sendmoney/' + item.icon" width='50'/> </td>
                                    ...
                                    <td class="act-btn">
                                        <a :href="'{{url(Session::get('panel').'/sendmoneylabels/edit')}}/'+ item.id" title="Edit"
...
<script>
    var indexUrlJson = "{{route('admin.sendmoneylabels.index_json',['panel' => Session::get('panel')])}}";
    var deleteUrl = "{{ url(Session::get('panel').'/sendmoneylabels/destroy') }}";
    var toggle_status_url = "{{url(Session::get('panel').'/sendmoneylabels/toggle-status')}}";
...
<script type="text/javascript" src="{{ asset('js/admin/send_money_labels.js') }}"></script>
@endpush