                ...
                <div class="d-flex">
                    <div class="justify-content-center">
                        ...
                        <a class="btn btn-primary my-2 btn-icon-text"
                            href="{{ route('admin.notifications.create',['panel' => Session::get('panel')]) }}">
                            <i class="fe fe-plus mr-2"></i> {{ @trans('notifications.add_notifications') }} </a>
                    </div>
                </div>
            </div>
            <!-- End Page Header -->
                                        ...
                                        <div class="table-checkable">
                                            <table id="example"
                                                ...
                                                <thead>
                                                    <tr>
                                                        <th>#</th>
                                                        <th><b>{{ @trans('notifications.username') }}</b><a
                                                                href="javascript:void(0)"
                                                                :class="classSort('users.first_name')"
                                                                @click="sort('users.first_name')"></a>
                                                        <th><b>{{ @trans('notifications.title') }}</b><a
                                                                href="javascript:void(0)" :class="classSort('title')"
                                                                @click="sort('title')"></a>
                                                        <th><b>Image</b><a href="javascript:void(0)"
                                                                :class="classSort('id')" @click="sort('id')"></a>
                                                        <th><b>{{ @trans('notifications.description') }}</b><a
                                                                href="javascript:void(0)"
                                                                :class="classSort('description')"
                                                                @click="sort('description')"></a>
                                                        </th>
                                                        <th><b>{{ @trans('notifications.action') }}</b></th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    <tr v-for="(item,index) in items">
                                                        ...
                                                        <td>@{{ item.title }}</td>
                                                        <td><img v-bind:src="item.image_name" width='50' /></td>
                                                        <td>@{{ item.description }}</td>
                                                        <td class="act-btn">
                                                            <!-- <a :href="'{{url(Session::get('panel').'/notifications/show')}}/'+ item.id" title="View"
                                                                class="btn-view btn-sm">
                                                                <i class="material-icons">remove_red_eye</i>
                                                            </a> -->
                                                            <!-- <a :href="'{{url(Session::get('panel').'/notifications/edit')}}/'+ item.id" title="Edit"
...
<script>
var user_indexUrlJson = "{{route('admin.notifications.index_json',['panel' => Session::get('panel')])}}";
var toggle_status_url = "{{url(Session::get('panel').'/users/toggle-status')}}";
var user_deleteUrl = "{{route('admin.notifications.destroy',['panel' => Session::get('panel')])}}";
