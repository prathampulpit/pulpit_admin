    ...
    <div class="container-fluid">
        <div class="inner-body">
            ...
            <div class="page-header">
                <div>
                    <h2 class="main-content-title tx-24 mg-b-5">{{ @trans('tripFare.local') }} {{ @trans('tripFare.tripFare') }} - {{ @trans('tripFare.website') }}</h2>
                    <!-- <ol class="breadcrumb">
                ...
                <div class="d-flex">
                    <div class="justify-content-center">
                        ...
                        <a class="btn btn-primary my-2 btn-icon-text" href="{{ route('admin.websiteTripFare.create',['panel' => Session::get('panel')]) }}">
                            <i class="fe fe-plus mr-2"></i>{{ @trans('tripFare.add') }} {{ @trans('tripFare.local') }} {{ @trans('tripFare.tripFare') }} </a>
                    </div>
                </div>
            </div>
            <!-- End Page Header -->
                                        ...
                                        <div class="table-checkable">
                                            <table id="example" class="table table-striped table-bordered text-nowrap dataTables_wrapper dt-bootstrap4 no-footer">
                                                <thead>
                                                    <tr>
                                                        <th>#</th>
                                                        <th><b>Vehicle</b><a href="javascript:void(0)"
                                                        :class="classSort('vehicle_type_id')" @click="sort('vehicle_type_id')"></a></th>
                                                        <th><b>Base Fare</b><a href="javascript:void(0)"
                                                                :class="classSort('base_fare')" @click="sort('base_fare')"></a></th>
<!--                                                        <th><b>KM range</b><a href="javascript:void(0)"
                                                                :class="classSort('km_range')" @click="sort('km_range')"></a>
                                                        </th>-->
                                                        <th><b>Per KM</b><a href="javascript:void(0)"
                                                                :class="classSort('per_km')" @click="sort('per_km')"></a>
                                                        </th>
                                                        <th><b>{{ @trans('tripFare.gst') }}</b><a href="javascript:void(0)"
                                                                :class="classSort('gst')" @click="sort('gst')">(%)</a>
                                                        </th>
                                                        <th><b>{{ @trans('tripFare.advance_booking') }}</b><a href="javascript:void(0)"
                                                                :class="classSort('advance_booking')" @click="sort('advance_booking')">(%)</a>
                                                        </th>
                                                ...
                                                <tbody>
                                                    <tr v-for="(item,index) in items">
                                                        ...
                                                        <!--<td>@{{ item.km_range }}/KMS</td>-->
                                                        <td>@{{ item.per_km }}/KM</td>
                                                        <td>@{{ item.gst }}%</td>
                                                        <td>@{{ item.advance_booking }}</td>
                                                        <td class="act-btn">                                
                                                            <a :href="'{{url(Session::get('panel').'/websiteTripFare/show')}}/'+ item.id" title="View"
                                                                class="btn-view btn-sm">
                                                                <i class="material-icons">remove_red_eye</i>
                                                            </a>                                        
                                                            <a :href="'{{url(Session::get('panel').'/websiteTripFare/edit')}}/'+ item.id" title="Edit"
...
<script>
    var user_indexUrlJson = "{{route('admin.websiteTripFare.index_json',['panel' => Session::get('panel')])}}";
    var toggle_status_url = "{{url(Session::get('panel').'/users/toggle-status')}}";
    var user_deleteUrl = "{{route('admin.websiteTripFare.destroy',['panel' => Session::get('panel')])}}";
...
<script type="text/javascript" src="{{ asset('js/admin/tripFare.js') }}"></script>
@endpush