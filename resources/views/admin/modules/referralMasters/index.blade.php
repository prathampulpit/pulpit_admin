...
@section('title') Referral Masters @endsection
            ...
            <div class="page-header">
                ...
                <div class="d-flex">
                    <div class="justify-content-center">
                        ...
                        <!-- <a class="btn btn-primary my-2 btn-icon-text" href="{{ route('admin.referralMasters.create',['panel' => Session::get('panel')]) }}">
                            <i class="fe fe-plus mr-2"></i> {{ @trans('referralMasters.add_referralMasters') }} </a> -->
                    </div>
                </div>
            </div>
            <!-- End Page Header -->
                                        ...
                                        <div class="table-checkable">
                                            <table id="example" class="table table-striped table-bordered text-nowrap dataTables_wrapper dt-bootstrap4 no-footer">
                                                <thead>
                                                    <tr>
                                                        <th>#</th>
                                                        <th><b>{{ @trans('referralMasters.type') }}</b><a href="javascript:void(0)"
                                                                :class="classSort('brand_id')" @click="sort('brand_id')"></a>
                                                        </th>
                                                        <th><b>{{ @trans('referralMasters.referral_bonus') }}</b><a href="javascript:void(0)"
                                                                :class="classSort('referral_bonus')" @click="sort('referral_bonus')"></a>
                                                        </th>
                                                        <th><b>{{ @trans('referralMasters.max_referral_bonus') }}</b><a href="javascript:void(0)"
                                                                :class="classSort('max_referral_bonus')" @click="sort('max_referral_bonus')"></a>
                                                        </th>
                                                        <th><b>{{ @trans('referralMasters.action') }}</b></th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    <tr v-for="(item,index) in items">
                                                        ...
                                                        <td v-if="item.type == 'Paid'">₹@{{ item.referral_bonus }}</td>
                                                        <td v-else>@{{ item.referral_bonus }} days</td>

                                                        <td v-if="item.type == 'Paid'">₹@{{ item.max_referral_bonus }}</td>
                                                        <td v-else>@{{ item.max_referral_bonus }} days</td>

                                                        <td class="act-btn">                                
                                                            <a :href="'{{url(Session::get('panel').'/referralMasters/edit')}}/'+ item.id" title="Edit"
...
<script>
    var user_indexUrlJson = "{{route('admin.referralMasters.index_json',['panel' => Session::get('panel')])}}";
    var toggle_status_url = "{{url(Session::get('panel').'/users/toggle-status')}}";
    var user_deleteUrl = "{{route('admin.referralMasters.destroy',['panel' => Session::get('panel')])}}";
...
<script type="text/javascript" src="{{ asset('js/admin/referralMasters.js') }}"></script>
@endpush